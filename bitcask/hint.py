...
def hint_to_bytes(hint: Hint) -> bytes:
    return (
        tstamp_to_bytes(hint.timestamp)
        + size_to_bytes(hint.ksz)
        + size_to_bytes(hint.value_sz)
        + pos_to_bytes(hint.value_pos)
        + hint.key
    )
...
def bytes_to_hint(b: bytes) -> Hint:
    ...
    return Hint(
        timestamp=timestamp, ksz=ksz, value_sz=value_sz, value_pos=value_pos, key=key
    )
